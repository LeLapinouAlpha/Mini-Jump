...
public class PlayerMovements : MonoBehaviour
{
    ...
    {
        // Check if player is grounded and update states
        this.isGrounded = Physics2D.OverlapCircle(this.groundCheck.position, this.groundCheckRadius, this.groundLayers);

        // Check if player is touching wall with two raycasts
        var isTouchingWallLeft = Physics2D.Raycast(this.wallCheckLeft.position, Vector2.left, this.wallCheckRadius, this.wallLayers);
        //Debug.DrawRay(this.wallCheckLeft.position, Vector2.left * this.wallCheckRadius, Color.blue, 0.5f);
        var isTouchingWallRight = Physics2D.Raycast(this.wallCheckRight.position, Vector2.right, this.wallCheckRadius, this.wallLayers);
        //Debug.DrawRay(this.wallCheckRight.position, Vector2.right * this.wallCheckRadius, Color.blue, 0.5f);
        this.isTouchingWall = isTouchingWallRight || isTouchingWallLeft;

        // Move player based on input and states
        this.MovePlayer();
    }
